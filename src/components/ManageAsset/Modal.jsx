...
const Modal = ({ showModal, setShowModal, type, asset, setIsReloadPage, showToastFromOut }) => {
    ...
    const handleDeleteAsset = async (assetCode) => {
        const deleteAssetRes = await axios.delete(`${AXIOS_API_URL}/admin/api/assets/${assetCode}`, 
            ...
            console.log(deleteAssetRes.data.message);
            setShowModal(false);
                        ...
                        <ModalWrapper showModal={showModal} id = "modal_wrapper">
                            ...
                            <ModalContent id = "modal_content">
                                ...
                                <DetailContent id = "detail_content">
                                    ...
                                    <DetailContentItem id = {asset.installedDate}>{DateFormatterService.dateFormat(asset.installedDate)}</DetailContentItem>
                                    <DetailContentItem id = {asset.state}>{AssetService.formatAssetState(asset.state)}</DetailContentItem>
                                    <DetailContentItem id = {asset.locationCode}>{asset.locationCode}</DetailContentItem>
                                    {/* <DetailContentItem id = {"location_"+asset.assetCode}>{asset.specification}</DetailContentItem> */}
                ...
                {showModal ? (
                    <Background ref={modalRef} onClick={closeModal}>
                        <ModalWrapper showModal={showModal} style={{ width: "400px" }} >
                            ...
                            <ModalContent style={{display: "flex", flexDirection: "column", alignItems: "flex-start", margin: "0"}}>
