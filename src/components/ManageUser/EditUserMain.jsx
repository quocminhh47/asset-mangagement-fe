...
const EditUserMain = (props) => {
    ...
    // UseEffect listen birthDate, joinedDate, form change
    useEffect(() => {
            ...
            <Form onChange={() => setIsChangeForm(prev => !prev)}>
                ...
                <FormContent>
                            ...
                            props.data.gender == true ?
                                <>
                                    ...
                                    <Label id='editUser_genderMale'>
                                        <InputRadio type="radio" name="genderUser" defaultChecked value={true} onChange={(e) => { setGender(e.target.value) }} />
                                ...
                                <>
                                    <Label id='editUser_genderFemale1'>
                                        <InputRadio type="radio" name="genderUser" defaultChecked value={false} onChange={(e) => { setGender(e.target.value) }} />
                                        <InputRadioSpan>Female</InputRadioSpan>
                                    </Label>
                    ...
                    <FormContentItem>
                        ...
                        ></InputDate>
                        {joinedDateSaturdaySundayError && <Error>Joined date is Saturday or Sunday.<br/> Please select a different date</Error>}
                        {joinedDateSoonError && <Error>Joined date is not later than Date of Birth.<br/> Please select a different date</Error>}
                    </FormContentItem>
                    <FormContentItem>
                            ...
                            } */}
                            {
                                ...
                                roles.map((role) => {
                                    if(role.name === props.data.roleName) {
                                        return (
                                            <Option value={role.name} selected>{StringFormatter.capitalizeFirstLetter(role.name)}</Option>
                                        );
                                    }
                                    return (
                                        <Option value={role.name}>{StringFormatter.capitalizeFirstLetter(role.name)}</Option>
                                    );
                                })
                                : null
                            }
